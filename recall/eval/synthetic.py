...
#: Written into every generated corpus. Its presence is what makes the directory safe to wipe on
#: regeneration — without it, a mistyped `--out` would delete a directory this module never made.
_MARKER = ".recall-synthetic"


def _prepare_root(root: Path) -> None:
    """Make `root` an empty directory owned by this generator, or refuse.

    A corpus is identified by a glob (`**/*.md`), so leftovers from a previous run are indexed
    as if they belonged to this one. That is silent: the returned `n_chunks` describes what this
    call generated, while the evaluation measures whatever the directory happens to contain. A
    run configured for 600 chunks silently indexed 20,600 that way — every latency and scale
    number attributed to the wrong corpus size.

    So regeneration WIPES the directory. It refuses on any directory not carrying `_MARKER`,
    because the alternative to a confusing result is not a destructive one: `--out ~/notes`
    must fail, not delete.
    """
    if root.exists():
        if any(root.iterdir()) and not (root / _MARKER).exists():
            raise FileExistsError(
                f"{root} is not empty and was not generated by recall.eval.synthetic "
                f"(no {_MARKER} marker). Refusing to overwrite it — point --out somewhere else."
            )
        shutil.rmtree(root)
    root.mkdir(parents=True)
    (root / _MARKER).write_text("generated by recall.eval.synthetic\n", encoding="utf-8")
