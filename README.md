...
| **Index generations and cutover** | ✅ registered generations, shadow route, dual write behind a durable ordered outbox, transactional route swap with a content-free `NOTIFY`; `cutover` refuses while any event is pending or the shadow is not ready | ⚠️ `parity` on two **empty** generations succeeds and prints `OK`: read the chunk counts it prints, because a vacuous pass and a real one look identical. A shadow attached midway through a corpus is also silently short, since the dual-write skip set is read from the active store |
