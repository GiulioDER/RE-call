...
| `enterprise_rag/dense_floor_strat100.retrieval.json` | the dense-cosine floor probe behind the Track B go/no-go — how many EnterpriseRAG questions the trust layer's **uncalibrated 0.50 threshold** would demote to `low_confidence` once the benchmark stops calling `HybridRetriever.search()` directly and goes through `reason()`. **At least 5.6% of 500**, so no mass abstention. ⚠️ **A LOWER BOUND, not an estimate**: `best_dense_score` is the corpus-wide dense top-1, while the floor is applied per *returned* hit over the post-fusion post-rerank top-k, and max(dense over the returned k) ≤ the corpus top-1 — a question counted safe here can still have no returned hit above the floor. ⚠️ **NOT a retrieval quality measurement** either: reranker and SPLADE are deliberately off, so `doc_ids_by_k` and `k_metrics` in that file do **not** describe the submitted arm and must not be read as recall. Sample is stratified 10-per-category, which overweights the small ones — the 16% sample rate is not the population rate. Two findings constrain the work: the loss is **concentrated** (60% of `miscellaneous` below the floor, a category already at 100.0 correctness, so gating it is a pure regression), and at 0.50 the floor abstains on **0 of 10** sampled `info_not_found` questions, so it pays 16% of the sample and catches none of the unanswerable ones. Mind the three denominators: 16/100 is over every sampled row, `threshold_metrics['0.5']`'s 13.75% is over the 80 rows carrying `expected_docs` and so excludes all 10 `high_level` and all 10 `info_not_found` rows, and 5.6% is over the 500-question population after reweighting. The summary is derived by `benchmarks/enterprise_rag_contract.summarize` and the runner writes through `write_dense_floor_artifact`, which validates first and leaves no file behind on mismatch |
