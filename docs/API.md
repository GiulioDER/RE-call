...
| `recall reasoning` | Inspect projections (`projection`), proposals (`proposals`), queries (`query`), traces (`trace`), audits (`audit`), and opt-in reasoning without changing ordinary retrieval behavior. Query and trace accept `--graph-expansion auto|off|one-hop`; `auto` is category aware. |
...
| `recall_reasoning_query` | Run an explicit opt-in reasoning query over trusted retrieval. `graph_expansion` defaults to `auto`, which enables one hop for multi hop, temporal, list completion, and explicit comparison queries while keeping numeric and direct single hop queries off. `off` and `one_hop` remain explicit overrides. Precision admission diagnostics and a policy fingerprint are additive response fields. Legacy `expand_retrieval` remains available when configured. |
...
reasoning is category aware by default, with conservative activation for selected query shapes. Set
