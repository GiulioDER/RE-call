...
Provider output is accepted only after shape, cardinality, generation, pipeline, relation,
confidence, and evidence citation validation. Timeout, malformed output, wrong cardinality, and
provider errors are returned in band. Rejected provider proposals are recorded separately rather
