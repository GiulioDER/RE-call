...
```text
.tmp_atm_vps2_bundle\recall-source-68021b9c.tar.gz
```

Its SHA256 is recorded by the transfer step and must be checked on VPS2 before extraction.
The current bundle SHA256 is `332D5F4FE474F252FD950CEEE32A4825A91FBC017DA0E36C0B91835EF7A2A6E1`.
...
```powershell
$bundle = Resolve-Path .tmp_atm_vps2_bundle\recall-source-68021b9c.tar.gz
scp $bundle vps2:/home/sentiment/atm-bench-run/recall-source-68021b9c.tar.gz
...
sha256sum recall-source-68021b9c.tar.gz
rm -rf source
mkdir source
tar -xzf recall-source-68021b9c.tar.gz -C source
```
