...
| **Schema migrations** | ✅ ordered SQL, committed cryptographic checksums, advisory lock, resumable concurrent indexes, separate migration/serving roles | MCP startup is SELECT-only and refuses pending, drifted, or unknown versions; supported PostgreSQL majors 16, 17 and 18 are tested |
...
| **Retrieval cost profiles** | ✅ `fast` and `quality` chosen per process, never per request; a request's `k` is clamped down and never raised; contradictory configuration refuses **startup**, not the first search; over-budget requests are shed at the door, before the query is embedded | ⚠️ Each profile's concurrency and queue depth is a stated policy choice, not a measurement, and cannot be tuned until the latency blocker below clears; the values are in `recall/profiles.py` and [docs/ENTERPRISE_RETRIEVAL.md](ENTERPRISE_RETRIEVAL.md). The reranker digest pins one provisioned **tree**, path names included, so it identifies a directory rather than the model in general |
