...
Each profile carries its **own** concurrency budget rather than one shared default. The fixed profiles deliberately narrow queue capacity as the per-request budget rises, so a slow reranked profile cannot accumulate the same waiting population as the fast profile. The fast and code values remain policy choices pending their own load tests. The quality profile's initial measured SLO and load boundary are recorded in [RETRIEVAL_SLO.md](RETRIEVAL_SLO.md). `RECALL_SEARCH_CONCURRENCY` and `RECALL_SEARCH_QUEUE` override them for the selected profile.
...
Two limits on what this says. It is a latency verdict, not a quality one: retrieval quality was never measured against `bge-small-asymmetric-v1`, so nothing here claims the model retrieves worse. The measurement used a fixed CPU configuration; GPU requirements are out of scope for this program, so a GPU number would not change the decision.
