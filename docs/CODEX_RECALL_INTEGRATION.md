...
The global Codex configuration at `C:\Users\gde00\.codex\hooks.json` mirrors Claude's
...
memory generation refresh independently from the docs and code refresh. If the automatic
`SessionEnd` hook has already run, pass `-NoRefresh` to avoid launching a duplicate refresh; the
explicit close command otherwise owns the refresh launch. Use `-NoMemo` only when the session
