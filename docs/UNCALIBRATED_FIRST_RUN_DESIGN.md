...
| `promote()` refuses in production, needs a flag otherwise <!-- cite-anchor: def promote --> | `recall/generations.py:1360` | 🔁 **no longer true.** Confirmed when written. `promote()` now admits a generation whose published calibration certified and is still bound, and `unsafe_development` is refused in production rather than being the other way through. See F2 |
...
`rollback()` (`recall/generations.py:1442`) <!-- cite-anchor: def rollback --> writes the same `active_generation_id` column with **no
environment check and no `unsafe_development` flag**, and its target may be in state `ready`
(`recall/generations.py:1486`) <!-- cite-anchor: GenerationState.RETIRED -->. So "no ungated generation becomes active in production" is not a
...
5. **Promotion permission.** Production once refused `promote()` outright; it now requires a published, certified, still-bound calibration (`recall/generations.py:1360`) <!-- cite-anchor: def promote -->. 🔁 Updated 2026-08-20.
6. **Generation creation.** Production requires a verified pipeline identity and refuses
   `allow_unverified` (`recall/generations.py:428`, `recall/generations.py:439`), which an adopted generation cannot satisfy with
...
design does not make. `rollback()` today takes no arguments (`recall/generations.py:1442`), so it
...
`forget()` that rewrote `corpus_fingerprint` (`recall/generations.py:1618`) or a `publish()` that
...
| `chunk_ordinal` | `int(metadata->>'ord')` | **Required alongside `content_hash` in step 1**; absent means not adoptable, matching `_write_source` (`recall/generations.py:678`) |
...
(`recall/generations.py:428`), and an adopted generation is unverified by construction. **Adoption
...
(`recall/generations.py:864`), and `gc()` already reclaims `failed`. The abandoned-generation
...
- **The body rule can move under it.** `parse_frontmatter` changed once, and the fix carries a
  version marker (`_BODY_RULE_VERSION`, `recall/generations.py:178`) precisely because the same bytes then yielded a
