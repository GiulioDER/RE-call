...
1. **Ingestion source.** Production refuses local filesystem indexing (`recall_mcp/service.py:2752`, `recall/cli_commands/index_search.py:254` <!-- cite-anchor: env_is_production -->).
...
**Decision: `recall/reasoning.py:1724` accepts `{trusted, degraded, refused, provisional}`, and
...
| `chunk_ordinal` | `int(metadata->>'ord')` | **Required alongside `content_hash` in step 1**; absent means not adoptable, matching `_write_source` (`recall/generations.py:727`) |
...
(`recall/generations.py:477`), and an adopted generation is unverified by construction. **Adoption
...
- **The body rule can move under it.** `parse_frontmatter` changed once, and the fix carries a
  version marker (`_BODY_RULE_VERSION`, `recall/generations.py:227`) precisely because the same bytes then yielded a
...
4. **Adding `provisional` to `TrustState` hits an exhaustive whitelist that raises**, not a defaulted
   mapping: `recall/reasoning.py:1724` rejects anything outside `{trusted, degraded, refused}`, on a
