...
1. **Ingestion source.** Production refuses local filesystem indexing (`recall_mcp/service.py:1844`, `recall/cli.py:2877`).
...
3. **Store class.** Production selects `GenerationStore`, at **three** sites, not one:
   `recall_mcp/server.py:660`, `recall/cli.py:2925`, and the `generation_mode` parameter threaded
