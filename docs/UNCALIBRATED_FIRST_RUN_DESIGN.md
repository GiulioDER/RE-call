...
| Claim | Site | Verdict |
|---|---|---|
| Server builds `GenerationStore` only in production <!-- cite-anchor: if generation_mode: --> | `recall_mcp/server.py:809` | confirmed. 🔁 **Line corrected twice: 2026-08-25 and again 2026-08-26.** It originally pointed 100-odd lines above the branch, at a `retrieval_profile` logging argument (the stale number is deliberately not written here in `path:line` form, because the anchor checker reads any such string as a live citation and a correction note would then fail the gate it exists to explain); the anchor is added so the next edit above it moves the pointer rather than silently invalidating it. The earlier correction landed one line short of the branch and disagreed with the citation of the SAME anchor at line 129 of this document. Both citation gates stayed green throughout, because the anchor window absorbs an off-by-one: a green gate is not evidence a pointer is right |
| Missing `generation_id` degrades to `"legacy"` | `recall_mcp/service.py:1768` | confirmed. A second site uses the same default but maps it to `None` immediately after, so the two do not behave identically |
| `promote()` refuses in production, needs a flag otherwise <!-- cite-anchor: def promote --> | `recall/generations.py:1244` | 🔁 **no longer true.** Confirmed when written. `promote()` now admits a generation whose published calibration certified and is still bound, and `unsafe_development` is refused in production rather than being the other way through. See F2 |
...
| `calibration = None` is deliberate, and names an open design question | `recall/cli_commands/index_search.py:560` | confirmed |
...
`rollback()` (`recall/generations.py:1297`) <!-- cite-anchor: def rollback --> writes the same `active_generation_id` column with **no
environment check and no `unsafe_development` flag**, and its target may be in state `ready`
(`recall/generations.py:1264`) <!-- cite-anchor: GenerationState.RETIRED -->. So "no ungated generation becomes active in production" is not a
...
It has no `source_sha256` column, but `recall/index.py:1010` stamps into every chunk's metadata:
...
`PgVectorStore.source_raw_hashes` (`recall/store.py:2830`), **not** `source_content_hashes`
(`recall/store.py:2812`), which coalesces `index_fingerprint` first and therefore returns the defective identifier.

⚠️ **`content_hash` is media type dependent since `bd582316`.** A markdown source is hashed as
decoded, newline normalised, `_strip_nul` text re encoded as UTF-8 (`recall/index.py:851`
and `recall/index.py:832`); any other media type is hashed as raw `source_bytes` (`recall/index.py:861`). Any adoption path must branch the
...
1. **Ingestion source.** Production refuses local filesystem indexing (`recall_mcp/service.py:3122`, `recall/cli_commands/index_search.py:254` <!-- cite-anchor: env_is_production -->).
...
3. **Store class.** Production selects `GenerationStore`, at **three** sites, not one:
    `recall_mcp/server.py:809` <!-- cite-anchor: if generation_mode: -->, `recall/cli_commands/index_search.py:346` <!-- cite-anchor: generation_mode -->, and the `generation_mode` parameter threaded
...
5. **Promotion permission.** Production once refused `promote()` outright; it now requires a published, certified, still-bound calibration (`recall/generations.py:1244`) <!-- cite-anchor: def promote -->. 🔁 Updated 2026-08-20.
...
design does not make. `rollback()` today takes no arguments (`recall/generations.py:1297`), so it
...
`DEPENDENCY_UNAVAILABLE` (`recall/trust.py:857`), whose advice text calls that condition "an
...
**Yes, and `recall/cli_commands/index_search.py:560` was right to refuse the version that would have been dishonest.**
...
3. Re derive the hash **exactly as the indexer does for that media type**: decoded, newline
   normalised, `_strip_nul` text for markdown (`recall/index.py:851`, `recall/index.py:832`), raw
   `source_bytes` otherwise (`recall/index.py:861`). Not equal means the file changed since indexing: not adoptable.
...
`embedding_profile(embedder).fingerprint()` (`recall/index.py:531`), which covers model name
...
`forget()` that rewrote `corpus_fingerprint` (`recall/generations.py:1405`) or a `publish()` that
...
**Decision: `recall/reasoning.py:1804` accepts `{trusted, degraded, refused, provisional}` <!-- cite-anchor: if value not in -->, and
...
(`recall/generations.py:951`), and `gc()` already reclaims `failed`. The abandoned-generation
...
`_index_fingerprint` carries no chunker CONFIGURATION either (`recall/index.py:478`), which is why
...
and the `Indexer`'s actual chunker (`recall/index.py:602`) never reaches it. Measured against
...
4. **Adding `provisional` to `TrustState` hits an exhaustive whitelist that raises**, not a defaulted
   mapping: `recall/reasoning.py:1804` <!-- cite-anchor: if value not in --> rejects anything outside `{trusted, degraded, refused}`, on a
...
5. **The strict gate is binary on `CERTIFIED`.** `code_for_status` returns a failure code for
   everything else via a fail closed default (`recall/trust_policy.py:112`), so a new
