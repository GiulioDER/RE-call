...
| `calibration = None` is deliberate, and names an open design question | `recall/cli.py:2233-2244` | confirmed |
...
1. **Ingestion source.** Production refuses local filesystem indexing (`recall_mcp/service.py:1837`, `recall/cli.py:2250`).
...
3. **Store class.** Production selects `GenerationStore`, at **three** sites, not one:
   `recall_mcp/server.py:629`, `recall/cli.py:2289`, and the `generation_mode` parameter threaded
...
design does not make. `rollback()` today takes no arguments (`recall/generations.py:1059`), so it
...
**Yes, and `recall/cli.py:2244` was right to refuse the version that would have been dishonest.**
...
superseded the artifact (`recall/calibration_v2.py:975`). A cached mode cannot catch either.
...
(`recall/calibration_v2.py:977` only refuses `not artifact.certified`). So certification and
