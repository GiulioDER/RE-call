...
1. **Ingestion source.** Production refuses local filesystem indexing (`recall_mcp/service.py:2506`, `recall/cli_commands/index_search.py:252` <!-- cite-anchor: env_is_production -->).
...
3. **Store class.** Production selects `GenerationStore`, at **three** sites, not one:
   `recall_mcp/server.py:780` <!-- cite-anchor: if generation_mode: -->, `recall/cli_commands/index_search.py:298` <!-- cite-anchor: generation_mode -->, and the `generation_mode` parameter threaded
