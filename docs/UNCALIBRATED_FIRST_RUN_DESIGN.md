...
| Missing `generation_id` is `null` in `SearchResult`, while the optional explanation labels it `"legacy"` | `recall_mcp/service.py:1037` | confirmed. The two fields intentionally preserve different compatibility contracts |
...
1. **Ingestion source.** Production refuses local filesystem indexing through the resolved route guard (`recall/cli_commands/index_search.py:305-309` <!-- cite-anchor: route.uses_generation -->).
