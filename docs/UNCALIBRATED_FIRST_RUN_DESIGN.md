...
| Legacy `chunks` has no `source_sha256` **column** | `recall/store.py:453` (`DEFAULT_TABLE`) vs `recall_chunks_v1` | confirmed as stated, and **narrower than "nothing to reuse"**: the metadata carries `content_hash`, which is what F3 is about |
...
(`recall/embeddings.py:1035`) now derives `unregistered__{model}__{dimension}__{kind}`
(`recall/embeddings.py:1035`) instead of claiming a registry id it does not have.
...
`PgVectorStore.source_raw_hashes` (`recall/store.py:3178`), **not** `source_content_hashes`
(`recall/store.py:3160`), which coalesces `index_fingerprint` first and therefore returns the defective identifier.
...
`DEPENDENCY_UNAVAILABLE` (`recall/trust.py:877`), whose advice text calls that condition "an
...
(`recall/embeddings.py:424`), so a field of that name is now in the hash. It is inert: it belongs to
...
(`recall/embeddings.py:426`) carries the inference library version as key material, and its
