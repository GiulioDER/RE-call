...
| Claim | Site | Verdict |
|---|---|---|
| Server builds `GenerationStore` only in production | `recall_mcp/server.py:660` | confirmed |
| Missing `generation_id` degrades to `"legacy"` | `recall_mcp/service.py:981` | confirmed. A second site uses the same default but maps it to `None` immediately after, so the two do not behave identically |
| `promote()` refuses in production, needs a flag otherwise <!-- cite-anchor: def promote --> | `recall/generations.py:1066` | 🔁 **no longer true.** Confirmed when written. `promote()` now admits a generation whose published calibration certified and is still bound, and `unsafe_development` is refused in production rather than being the other way through. See F2 |
...
| Legacy `chunks` has no `source_sha256` **column** | `recall/store.py:371` (`DEFAULT_TABLE`) vs `recall_chunks_v1` | confirmed as stated, and **narrower than "nothing to reuse"**: the metadata carries `content_hash`, which is what F3 is about |
...
`rollback()` (`recall/generations.py:1084`) <!-- cite-anchor: def rollback --> writes the same `active_generation_id` column with **no
environment check and no `unsafe_development` flag**, and its target may be in state `ready`
(`recall/generations.py:1192`) <!-- cite-anchor: GenerationState.RETIRED -->. So "no ungated generation becomes active in production" is not a
...
It has no `source_sha256` column, but `recall/index.py:946` stamps into every chunk's metadata:
...
decoded, newline normalised, NUL stripped text re encoded as UTF-8 (`recall/index.py:821` and
...
1. **Ingestion source.** Production refuses local filesystem indexing (`recall_mcp/service.py:1844`, `recall/cli_commands/index_search.py:205`).
2. **Auth.** Production refuses static bearer tokens (`recall_mcp/auth.py:375`).
3. **Store class.** Production selects `GenerationStore`, at **three** sites, not one:
   `recall_mcp/server.py:660`, `recall/cli_commands/index_search.py:256`, and the `generation_mode` parameter threaded
...
5. **Promotion permission.** Production once refused `promote()` outright; it now requires a published, certified, still-bound calibration (`recall/generations.py:1066`) <!-- cite-anchor: def promote -->. 🔁 Updated 2026-08-20.
6. **Generation creation.** Production requires a verified pipeline identity and refuses
   `allow_unverified` (`recall/generations.py:345`, `:321`), which an adopted generation cannot satisfy with
...
design does not make. `rollback()` today takes no arguments (`recall/generations.py:1084`), so it
...
3. Re derive the hash **exactly as the indexer does for that media type**: decoded, newline
   normalised, NUL stripped text for markdown (`recall/index.py:821`, `:798`), raw bytes otherwise
...
`forget()` that rewrote `corpus_fingerprint` (`recall/generations.py:1324`) or a `publish()` that
...
**Decision: `recall/reasoning.py:1464` accepts `{trusted, degraded, refused, provisional}`, and
...
| `chunk_ordinal` | `int(metadata->>'ord')` | **Required alongside `content_hash` in step 1**; absent means not adoptable, matching `_write_source` (`recall/generations.py:589`) |
...
(`recall/generations.py:346`), and an adopted generation is unverified by construction. **Adoption
...
(`recall/generations.py:748`), and `gc()` already reclaims `failed`. The abandoned-generation
...
4. **Adding `provisional` to `TrustState` hits an exhaustive whitelist that raises**, not a defaulted
   mapping: `recall/reasoning.py:1464` rejects anything outside `{trusted, degraded, refused}`, on a
