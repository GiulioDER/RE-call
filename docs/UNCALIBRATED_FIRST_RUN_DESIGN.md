...
| Missing `generation_id` is `null` in `SearchResult`, while the optional explanation labels it `"legacy"` | `recall_mcp/service.py:954` | confirmed. The two fields intentionally preserve different compatibility contracts |
...
1. **Ingestion source.** Production refuses local filesystem indexing through the resolved route guard (`recall_mcp/service.py:3186`, `recall/cli_commands/index_search.py:303-309` <!-- cite-anchor: route.uses_generation -->).
