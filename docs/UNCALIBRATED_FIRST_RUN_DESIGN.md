...
1. **Ingestion source.** Production refuses local filesystem indexing (`recall_mcp/service.py:1836`, `recall/cli.py:2365`).
...
3. **Store class.** Production selects `GenerationStore`, at **three** sites, not one:
   `recall_mcp/server.py:629`, `recall/cli.py:2511`, and the `generation_mode` parameter threaded
