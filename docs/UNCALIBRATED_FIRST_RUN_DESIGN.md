...
`PgVectorStore.source_raw_hashes` (`recall/store.py:2313`), **not** `source_content_hashes`
(`recall/store.py:2295`), which coalesces `index_fingerprint` first and therefore returns the defective identifier.
