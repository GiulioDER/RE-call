...
| Claim | Site | Verdict |
|---|---|---|
| Server builds `GenerationStore` only when the resolved route uses generation <!-- cite-anchor: if generation_mode: --> | `recall_mcp/server.py:944` | confirmed. The route is resolved once at startup and both serving and writes use that decision |
...
| `promote()` refuses in production, needs a flag otherwise <!-- cite-anchor: def promote --> | `recall/generations.py:1393` | 🔁 **no longer true.** Confirmed when written. `promote()` now admits a generation whose published calibration certified and is still bound, and `unsafe_development` is refused in production rather than being the other way through. See F2 |
...
`rollback()` (`recall/generations.py:1446`) <!-- cite-anchor: def rollback --> writes the same `active_generation_id` column with **no
environment check and no `unsafe_development` flag**, and its target may be in state `ready`
(`recall/generations.py:1407`) <!-- cite-anchor: GenerationState.RETIRED -->. So "no ungated generation becomes active in production" is not a
...
`PgVectorStore.source_raw_hashes` (in `recall/store.py`), **not** `source_content_hashes`
...
3. **Store class.** Production selects `GenerationStore`, at **three** sites, not one:
    `recall_mcp/server.py:944` <!-- cite-anchor: if generation_mode: -->, `recall/cli_commands/index_search.py:385` <!-- cite-anchor: generation_mode -->, and the `generation_mode` parameter threaded
...
5. **Promotion permission.** Production once refused `promote()` outright; it now requires a published, certified, still-bound calibration (`recall/generations.py:1392`) <!-- cite-anchor: def promote -->. 🔁 Updated 2026-08-20.
6. **Generation creation.** Production requires a verified pipeline identity and refuses
   `allow_unverified` (`recall/generations.py:415`, `recall/generations.py:427`), which an adopted generation cannot satisfy with
...
design does not make. `rollback()` today takes no arguments (`recall/generations.py:1446`), so it
...
`DEPENDENCY_UNAVAILABLE` (in `recall/trust.py`), whose advice text calls that condition "an
...
`forget()` that rewrote `corpus_fingerprint` (`recall/generations.py:1553`) or a `publish()` that
...
(`recall/generations.py:410`), and an adopted generation is unverified by construction. **Adoption
...
(`recall/generations.py:1062`), and `gc()` already reclaims `failed`. The abandoned-generation
...
- **The body rule can move under it.** `parse_frontmatter` changed once, and the fix carries a
  version marker (`_BODY_RULE_VERSION`, `recall/generations.py:185`) precisely because the same bytes then yielded a
