...
| `promote()` refuses in production, needs a flag otherwise <!-- cite-anchor: def promote --> | `recall/generations.py:1158` | 🔁 **no longer true.** Confirmed when written. `promote()` now admits a generation whose published calibration certified and is still bound, and `unsafe_development` is refused in production rather than being the other way through. See F2 |
...
(`recall/generation_store.py:147`). And `SERVABLE_ACTIVE_STATES = frozenset({"ready", "active"})`
...
(`recall/generations.py:1284`) <!-- cite-anchor: GenerationState.RETIRED -->. So "no ungated generation becomes active in production" is not a
...
5. **Promotion permission.** Production once refused `promote()` outright; it now requires a published, certified, still-bound calibration (`recall/generations.py:1158`) <!-- cite-anchor: def promote -->. 🔁 Updated 2026-08-20.
6. **Generation creation.** Production requires a verified pipeline identity and refuses
   `allow_unverified` (`recall/generations.py:367`, `recall/generations.py:372`), which an adopted generation cannot satisfy with
...
`forget()` that rewrote `corpus_fingerprint` (`recall/generations.py:1325`) or a `publish()` that
...
**Decision: `recall/reasoning.py:1548` accepts `{trusted, degraded, refused, provisional}`, and
...
| `chunk_ordinal` | `int(metadata->>'ord')` | **Required alongside `content_hash` in step 1**; absent means not adoptable, matching `_write_source` (`recall/generations.py:590`) |
...
(`recall/generations.py:367`), and an adopted generation is unverified by construction. **Adoption
...
(`recall/generations.py:749`), and `gc()` already reclaims `failed`. The abandoned-generation
...
- **The body rule can move under it.** `parse_frontmatter` changed once, and the fix carries a
  version marker (`_BODY_RULE_VERSION`, `recall/generations.py:120`) precisely because the same bytes then yielded a
...
4. **Adding `provisional` to `TrustState` hits an exhaustive whitelist that raises**, not a defaulted
   mapping: `recall/reasoning.py:1548` rejects anything outside `{trusted, degraded, refused}`, on a
