...
with or without the block, so `text_start` / `text_end` (`recall/index.py:866`) are invariant.
...
| `recall/generations.py:603` | Already inside the `media_type in {"text/markdown", ...}` branch, so non-markdown sources are untouched by construction. **Not optional:** `recall index` is refused under `RECALL_ENV=production` (`recall/cli.py:2341`), so hooking only the index path leaves the one build path that runs in production uncovered. |
