...
| `recall/generations.py:603` | Already inside the `media_type in {"text/markdown", ...}` branch, so non-markdown sources are untouched by construction. **Not optional:** `recall index` is refused under `RECALL_ENV=production` (`recall/cli.py:2650`), so hooking only the index path leaves the one build path that runs in production uncovered. |
