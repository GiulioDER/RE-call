...
(`recall/index.py:788`), and for everything else over the raw bytes (`recall/index.py:790`). Writing
...
with or without the block, so `text_start` / `text_end` (`recall/index.py:916`) are invariant.
...
`digest:` is `canonical_sha256` (`recall/lineage.py:131`) over `{"v": 1, "entries": [...]}`.
...
block (`recall/index.py:250`), so embeddings serve from cache (`recall/cache.py:400`); the cost is
...
`chunk_text` and `chunk_code` today only because `_pack` (`recall/index.py:250`) strips each block
...
| Site | Note |
|---|---|
| `recall/index.py:981` | `contextual_passages(raw, body, ...)` keeps taking the unstripped `raw` for `document_title`, which reads frontmatter and the first H1 — both above the block. The `body` argument becomes `human_body`. |
| `recall/generations.py:786` | Already inside the `media_type in {"text/markdown", ...}` branch, so non-markdown sources are untouched by construction. **Not optional:** `recall index` is refused on the generation route via `route.uses_generation` (`recall/cli_commands/index_search.py:290`) <!-- cite-anchor: route.uses_generation -->, so hooking only the index path leaves the one build path that runs in production uncovered. |
