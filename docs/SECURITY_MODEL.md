...
model locally (`pip install "recall-rag[fastembed]"`) and never makes a network call with chunk
text. This is the default — and, as shipped, the *only* backend `recall_mcp/server.py` and
