...
| **Retrieval leads a third-party personal-memory board, scored by its own evaluator** | On **ATM-Bench**, 1,013 <!--@ atm/atm_bench_full_20260821.json # question_count --> personal-memory QA questions over 11,034 <!--@ atm/atm_bench_full_20260821.json # corpus_items --> email, image and video items: **Recall@10 92.8924** <!--@ atm/atm_bench_full_20260821.json # retrieval.recall_at_10_percent -->, against 79.09 <!--@ citation-pending: the board's published Memexa row, reproduced in docs/ATM_BENCH.md, not a RE-call measurement --> for the best published row, and **QS 68.4264** <!--@ atm/atm_bench_full_20260821.json # official_score.qs_percent --> from the benchmark's own evaluator → [ATM_BENCH.md](ATM_BENCH.md) | ✅ **Accepted onto the board 2026-08-23**, under `RAG`. The QS column is still **not answer-model-matched**: the baselines answer with `Qwen3-VL-8B-Instruct` and this run answers with `DeepSeek V4 Pro`, so part of the QS lead is bought by a stronger reader. Recall is judge- and answerer-independent and is the like-for-like column. The judge kept the official prompt and model identity over a **non-official transport**, disclosed and accepted by the maintainers on 2026-08-23. The harness that produced the run is committed here byte for byte since 2026-08-23 and pinned by a test, so the code is checkable; `recall/` itself has moved since, so a re-execution reproduces the method rather than the last decimal |
