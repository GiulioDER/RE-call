...
ENVIRONMENT_SCHEMA: tuple[EnvironmentSpec, ...] = (
    ...
    EnvironmentSpec("RECALL_ATOMIC_RESCUE_MODE", "Retrieval", "off or sampled shadow atomic rescue", "off"),
...
def _validate_runtime_options(source: Mapping[str, str]) -> None:
    ...
    if source.get("RECALL_ATOMIC_RESCUE_MODE", "off").strip().lower() not in {
        "off",
        "shadow",
    }:
        raise ValueError("RECALL_ATOMIC_RESCUE_MODE must be off or shadow")
