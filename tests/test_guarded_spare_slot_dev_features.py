...
from scripts.capture_guarded_spare_slot_dev_features import analyze_query, try_analyze_query
...
def test_dev_features_records_selection_mismatch_without_a_feature_row() -> None:
    """Red proof for ``try_analyze_query``: the first mismatch aborted all later rows."""
    artifact = load_source_conditioning_artifact(
        Path("docs/results/2026-09-13-source-conditioning-model.json")
    )
    item = _item("gold:0", "gold.md")
    ...
    capture = {"base_hashes": [], "candidate_hashes": []}
    pool_audit = {"items": [item]}
    legs = {
        "dense": [{"chunk_id": "gold:0", "source": "gold.md", "rank": 1, "cosine": 0.4}],
        "sparse": [{"chunk_id": "gold:0", "source": "gold.md", "rank": 1, "cosine": 0.4}],
    ...
    row, reason = try_analyze_query(query, capture, pool_audit, legs, artifact)

    assert row is None
    assert reason == "selection_parity"
