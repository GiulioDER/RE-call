...
class _Store:
    def __init__(self) -> None:
        self.sparse_calls = 0
        self.chunks = [
            Chunk("dense", "source", "unrelated dense candidate", {}),
            Chunk("lexical", "source", "parser parser regression fix", {}),
        ]
...
def test_code4_profile_and_variant_freeze_the_promoted_candidate() -> None:
    ...
    behavior = variant("C5_code4_bm25")
...
def test_code4_add_windows_the_rendered_session_across_message_boundaries() -> None:
    request = AddRequest(
        request_id="request",
        user_id="user",
        session_id="session",
    ...
    assert [chunk.metadata["word_start"] for chunk in chunks] == [0, 4, 8]
...
def test_code4_retrieval_uses_canonical_bm25_not_postgres_sparse() -> None:
    ...
    assert [hit.chunk.id for hit in run.hits] == ["dense", "lexical"]
