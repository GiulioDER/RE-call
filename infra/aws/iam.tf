...
resource "aws_iam_role_policy" "ecs_task" {
  ...
  policy = jsonencode({ Version = "2012-10-17", Statement = [
    ...
    { Effect = "Allow", Action = ["secretsmanager:GetSecretValue", "secretsmanager:DescribeSecret"], Resource = compact([var.oidc_secret_arn, var.provider_secret_arn, aws_rds_cluster.this.master_user_secret[0].secret_arn]) },
    { Effect = "Allow", Action = ["kms:Decrypt", "kms:Encrypt", "kms:GenerateDataKey"], Resource = [coalesce(var.kms_key_arn, aws_kms_key.this.arn)] },
    { Effect = "Allow", Action = ["rds:RestoreDBClusterToPointInTime", "rds:DescribeDBClusters", "rds:DescribeDBClusterSnapshots"], Resource = ["*"] }
  ] })
}
