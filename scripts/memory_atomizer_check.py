    ...
    *,
    ...
    for source in eligible[:sources]:
        ...
        for _ in range(50):
            ...
            if sum(1 for word in span_words if _ALNUM.search(word)) >= ref.MIN_SPAN_CONTENT_WORDS:
                break
        else:
            continue
...
def generate_memory_probes(
    ...
    *,
    ...
    spans = sample_memory_spans(chunks, excluded)
    ...
    with out.open("w", encoding="utf-8", newline="\n") as handle:
        for span in spans:
            ...
            context = chunk_text.replace(span.text, f"[[ {span.text} ]]", 1) if span.text in chunk_text else chunk_text
            payload = {
                "model": round3.WRITER_MODEL,
                "provider": round3.WRITER_ROUTING,
                "temperature": 0,
                "max_tokens": 200,
                "response_format": ref.RESPONSE_FORMAT,
                "messages": [
                    {"role": "system", "content": MEMORY_PROMPT},
                    {"role": "user", "content": json.dumps({"CONTEXT": context, "TARGET": span.text}, ensure_ascii=False)},
                ],
                "usage": {"include": True},
            }
            ...
            handle.write(
                json.dumps(
                    {
                        "probe_id": "mem-" + hashlib.sha256(f"{span.chunk_id}\x00{span.text}".encode("utf-8")).hexdigest()[:16],
                        "source": span.source,
...
def micro_chunk_views(chunks: Sequence[StoredChunk]) -> list[tuple[StoredChunk, int, str]]:
    ...
    for chunk in sorted(chunks, key=lambda item: (item.source, item.ordinal)):
        words = chunk.text.split()
        for start, end in ref.window_bounds(len(words), size=MICRO_SIZE, stride=MICRO_STRIDE):
...
def main() -> None:
    ...
    probes.add_argument("--evals-root", type=Path, required=True)
    ...
    if args.command == "probes":
        ...
        summary["excluded_sources"] = len(excluded_sources(records))
    else:
