...
This record is frozen before the rerun. Earlier smoke artifacts remain preserved. This run makes
the Windows OpenSSH executable and SSH configuration path explicit for the RE-call MCP child.

## Prediction

The `recall_on` arm should make exactly one RE-call MCP search for the specified phrase. The
`recall_off` arm should make zero RE-call MCP calls. Both arms should complete the same task.
Tokens and wall time remain descriptive only.

## Fixed configuration

| Field | Value |
|---|---|
| Run identifier | `smoke-2026-08-19-rerun-03` |
| Task count | `1` |
...
| RE-call on configuration | Temporary `CODEX_HOME`, explicit OpenSSH path, explicit SSH config, and `recall-memory` MCP server. |
