...
**held-out** query split (so it's generalization, not memorization).
    ...
    python benchmarks/finetune/train.py --epochs 8

Output: base vs fine-tuned test MRR / nDCG@10 and the delta. The fine-tuned model is saved to
`benchmarks/finetune/model/` (gitignored — we commit the numbers, not the weights).
...
python benchmarks/finetune/train.py --corpus benchmarks/finetune/confusable_corpus \
  --queries benchmarks/finetune/confusable_queries.json --epochs 10 --out benchmarks/finetune/model_confusable
```
