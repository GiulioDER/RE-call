# Codex and RE-call smoke rerun

This record is frozen before the rerun. The first launch with the original record is preserved as
an invalid harness attempt because normal Codex stderr was incorrectly treated as a failure.

## Prediction

For the same read-only task, both arms should complete successfully and produce the same three
requested facts. The `recall_on` arm should make exactly one RE-call MCP search for the specified
phrase. The `recall_off` arm should make zero RE-call MCP calls. Token counts and wall time are
...
| Field | Value |
|---|---|
| Run identifier | `smoke-2026-08-19-rerun-01` |
| Task count | `1` |
