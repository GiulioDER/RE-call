...
VENDORED VERBATIM, and that is the point
----------------------------------------
This file is copied byte-for-byte from ``benchmarks/beam/prompts.py`` in
https://github.com/mem0ai/memory-benchmarks (Apache License 2.0), the harness Mem0 published its
own BEAM numbers with. Only this header is added.

A BEAM score is a property of the (retrieval, answerer, judge) triple, not of retrieval alone: the
answerer's instruction to say "I don't have enough information to answer this question." and the
judge's 0/0.5/1.0 rubric both move the number. Paraphrasing either — even improving it — would
make our RE-call cells incomparable with Mem0's published 64.1 (1M) / 48.6 (10M) while still
looking like the same benchmark, which is the failure mode this vendoring exists to prevent.

So: do NOT edit the prompt bodies. If upstream changes them, re-copy the file wholesale and re-run
both arms — a prompt edit is a re-run, not a patch. The upstream commit these were taken from is
recorded in ``benchmarks/beam/UPSTREAM.md`` alongside the vendoring date.
"""
...
def get_beam_answer_generation_prompt(
    question: str, memories: list, top_k: int | None = None
) -> str:
